...
- **זוס** (Zeus) — **ז**ו**ס** → ז+ס = 00 ✓ | same as HE-A; ז=0, ס=0
  - **A:** electrocutes the target with one raised hand
  - **O:** glowing golden zigzag thunderbolt

![Zeus](images/00_zeus.png)
...
- **סטלון** (Sylvester Stallone) — **ס**ט → ס+ט = 01 ✓ | Rocky's fists, Rambo
  - **A:** uppercuts the target into the air
  - **O:** oversized red boxing glove

![Stallone](images/01_stallone.png)

- **זידאן** (Zinedine Zidane) — **ז**ד → ז+ד = 01 ✓ | soccer GOAT, headbutt in 2006 World Cup final *(⚠️ HE-A = 04)*
  - **A:** headbutts the target full force
...
- **סנטה קלאוס** (Santa Claus) — **ס**נ → ס+נ = 02 ✓ | same as HE-A
  - **A:** shoves the target down a chimney
  - **O:** bulging red gift sack

![Santa Claus](images/02_santa.png)

- **סוניק הקיפוד** (Sonic) — **ס**נ → ס+נ = 02 ✓ | blue blur, gold rings, attitude
  - **A:** spin-dashes through the target
  - **O:** giant floating gold ring

![Sonic](images/02_sonic.png)
...
- **סמואל ל. ג'קסון** (Samuel L. Jackson) — **ס**מ → ס+מ = 03 ✓ | "Say what again!", purple lightsaber *(⚠️ HE-A = 08; matches EN slot 03)*
  - **A:** points the target inches from your face, screaming "SAY WHAT AGAIN!"
  - **O:** purple lightsaber

- **סמוראי** (Samurai) — **ס**מ → ס+מ = 03 ✓ | katana, black armor, deadly stance
  - **A:** splits the target in one clean sword stroke
  - **O:** black samurai helmet

![Samurai](images/03_samurai.png)
...
- **זורו** (Zorro) — **ז**ר → ז+ר = 04 ✓ | masked hero, black outfit, Z slashed everywhere *(⚠️ HE-A = 07)*
  - **A:** carves a giant Z into the target
  - **O:** black domino mask

![Zorro](images/04_zorro.png)
...
- **סלבדור דאלי** (Salvador Dalí) — **ס**ל → ס+ל = 05 ✓ | melting clocks, insane moustache *(⚠️ HE-A = 01)*
  - **A:** melts the target over the edge of a table
  - **O:** drooping melting clock

![Salvador Dalí](images/05_dali.png)

- **סול גודמן** (Saul Goodman) — **ס**ל → ס+ל = 05 ✓ | shady lawyer, flashy suits, "Better Call Saul" *(⚠️ HE-A = 01)*
  - **A:** slaps a contract onto the target and winks
  - **O:** yellow "Better Call Saul" billboard

![Saul Goodman](images/05_saul_goodman.png)
...
- **סאשה ברון כהן** — **ס**ש → ס+ש = 06 ✓ | same as HE-A
  - **A:** awkwardly thrusts the target at someone, completely stone-faced
  - **O:** Borat's grey suit
...
- **סוקרטס** (Socrates) — **ס**ק → ס+ק = 07 ✓ | barefoot philosopher, hemlock, Socratic method *(⚠️ HE-A = 03)*
  - **A:** calmly poisons the target with hemlock
  - **O:** plain clay cup of hemlock

![Socrates](images/07_socrates.png)

- **סקארלט** (Scarlett Johansson) — **ס**ק → ס+ק = 07 ✓
  - **A:** scissor-flips the target to the ground
  - **O:** Black Widow wrist gauntlets

![Scarlett Johansson](images/07_scarlett.png)
...
- **סופיה ורגרה** (Sofia Vergara) — **ס**ו → ס+ו(v) = 08 ✓ | Gloria from Modern Family, accent so thick it became the whole character
  - **A:** shouts the target into stunned silence
  - **O:** phone with huge red lipstick mark

![Sofia Vergara](images/08_sofia_vergara.png)

- **זאב ז'בוטינסקי** (Ze'ev Jabotinsky) — **ז**+**ב**(v) = 08 ✓ | iron wall doctrine, never backed down — זאב (wolf) = ז+ב, same encoding
  - **A:** pounds the target on a podium
  - **O:** rolled-up "Iron Wall" pamphlet

![Ze'ev / Wolf](images/08_wolf.png)
...
- **ספיידר-מן** (Spider-Man) — **ס**פּ → ס+פ(p) = 09 ✓ | web-slinging, red-and-blue suit, "With great power…"
  - **A:** webs the target point-blank
  - **O:** web shooter on wrist

![Spider-Man](images/09_spiderman.png)
